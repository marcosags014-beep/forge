...
export default function DailyQuoteModal() {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-6 transition-opacity duration-350 ${animOut ? 'opacity-0' : 'opacity-100'}`}
      style={{ background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(8px)' }}
      onClick={dismiss}
    >
      <div
        className={`max-w-lg w-full flex flex-col items-center text-center gap-6 transition-all duration-350 ${animOut ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
